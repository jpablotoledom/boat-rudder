...
void cms_get_categories(const char *lang, CmsCategoryItem **out, size_t *out_count) {
    ...
    *out_count = 0;
    ...
    bson_t *opts = BCON_NEW(
        ...
        "limit", BCON_INT64((int64_t)CATEGORY_LIST_LIMIT)
    );

    mongoc_cursor_t *cursor = mongoc_collection_find_with_opts(collection, query, opts, NULL);

    CmsCategoryItem *items = calloc(CATEGORY_LIST_LIMIT, sizeof(CmsCategoryItem));
    if (!items) {
        bson_destroy(query);
        bson_destroy(opts);
    ...
    size_t count = 0;
    const bson_t *doc;
    while (count < CATEGORY_LIST_LIMIT && mongoc_cursor_next(cursor, &doc)) {
        bson_iter_t iter;
    ...
    if (mongoc_cursor_error(cursor, &error))
        LOG_ERROR("cms_get_categories: cursor error: %s", error.message);
...
int cms_get_category_name_values(const char *id_hex, const CmsLanguageItem *langs,
    ...
    bson_error_t error;
    if (mongoc_cursor_error(cursor, &error))
        LOG_ERROR("cms_get_category_name_values: cursor error: %s", error.message);
...
int cms_create_category(const CmsLanguageItem *langs, size_t lang_count, char *const *values) {
    ...
    if (!ok) LOG_ERROR("cms_create_category: insert failed: %s", error.message);
...
int cms_update_category(const char *id_hex, const CmsLanguageItem *langs,
    ...
    if (!ok) LOG_ERROR("cms_update_category: update failed: %s", error.message);
...
int cms_delete_category(const char *id_hex) {
    ...
    if (!ok) LOG_ERROR("cms_delete_category: delete failed: %s", error.message);
